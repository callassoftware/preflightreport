...
function getDate(){
	...
	s = padDigits(d.getUTCDate(),2) + "." + padDigits(d.getUTCMonth(),2) + "." + d.getUTCFullYear() + " " + padDigits(d.getUTCHours(),2) + ":" + padDigits(d.getUTCMinutes(),2);
	return s;
}
...
function addOverviewHitIcons( parent )
{
	...
	if (sShowDetailsHits_Errors && (sShowOverviewIconIfNoHitsOfThatType || (err>0))) {
		addOverviewHitIcon( parent, err, "img/error.pdf"  , "img/error_g.pdf"  , "-" );
		sum += err;
	}
	if (sShowDetailsHits_Warnings && (sShowOverviewIconIfNoHitsOfThatType || (wrn>0))) {
		addOverviewHitIcon( parent, wrn, "img/warning.pdf", "img/warning_g.pdf", "-" );
		sum += wrn;
	}
	if (sShowDetailsHits_Infos && (sShowOverviewIconIfNoHitsOfThatType || (inf>0))) {
