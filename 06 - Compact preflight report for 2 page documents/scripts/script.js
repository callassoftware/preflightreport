...
function updatePreviewImages() {
	...
	// How many pages in our document?
	var numPages = getNumPages();

	// We do one thing if we have a single page, another if it's 2 or more
	if (numPages == 1) {

		// Set the single image
		image1.attr("src", cals_doc_info.docs[0].pages[0].page_img);
		
		// Hide the additional image container
		$( "td.column2" ).hide();
		...
		// Set the images
		image1.attr("src", cals_doc_info.docs[0].pages[0].page_img);
		image2.attr("src", cals_doc_info.docs[0].pages[1].page_img);
